...
function NagStreamerBody({
  ...
  let extraBitsBalanceInfo = (
    <React.Fragment>
      <p>
        ...
        <b>
          {Math.floor(activationInfo.balance / TRANSLATION_COST)} stream day(s)
        </b>
        .
      </p>
      <p>
  ...
  const languageList = useMemo(() => {
    ...
    return languageKeys.map((langKey) => (
      <li key={langKey}>{activationInfo.languages[langKey]}</li>
    ))
  ...
  return (
    <div data-testid="nag-streamer" className={Classes.DIALOG_BODY}>
      <p>
        The broadcaster currently does not have <b>Stream Closed Captioner</b>{' '}
        turned on.
      </p>
      {extraBitsBalanceInfo}
      <p>
        Let the broadcaster know you would like them to turn on{' '}
        <b>Stream Closed Captioner</b> so you can see{' '}
        <b>Translated Closed Captions</b> by visiting{' '}
        <a href="https://stream-cc.gooseman.codes">
      ...
      <ul>{languageList}</ul>
      <p>
        You can add more translation stream days by selecting an option below
        and click Submit.
      </p>
      <Select
...
export default connect(mapStateToProps, mapDispatchToProps)(NagStreamerBody)
